...
class LotusTerminal(Workspace):
    ...
    def _get_session(self, cwd : str) -> Optional[Popen]:
        ...
        try:
            out = self.far_end
            shell_session = subprocess.Popen(['gnome-terminal', '--working-directory=' + cwd],
                                             stdin=PIPE, stdout=out, stderr=out, text=True, cwd=cwd)
    ...
    def run(self, command : str) :
        new_text = command + '\n'
        self.session.stdin.write(new_text)
    ...
    def _read_pipe(self):
        try:
            while True:
                self.text += f'{os.read(self.near_end, 1024).decode()}\n'
