import os.path
import subprocess
import sys

from eval.uniteval import UnitEval
from holytools.devtools import Unittest


class WorkflowEval(UnitEval):
    def evaluate_unittest(self, unittest : type[Unittest]):
        script_fpath = sys.modules[unittest.__module__].__file__
        script_fname = os.path.basename(script_fpath)

        testenv_script_fpath = os.path.join(self.frame_dirpath,script_fname)
        print(f'-Script dirpath = {testenv_script_fpath}')

        # input(f'Implement the solution if you please!'
        #       f'Module can be found at {os.path.join(self.proj_dirpath, 'build.py')}')

        env = os.environ.copy()
        env['PYTHONPATH'] = f'{self.testenv_dirpath}'
        print(env['PYTHONPATH'])

        moduel_fname = script_fname.replace('.py', '')
        process = subprocess.Popen([self.testenv_python_fpath, '-m' f'pkg.frame.{moduel_fname}'], env=env)
        exit_code = process.wait()

        return exit_code == 0

if __name__ == '__main__':
    from eval.workflow.frame import TestNVDATicker
    wf_eval = WorkflowEval.ready()
    wf_eval.evaluate_unittest(unittest=TestNVDATicker)