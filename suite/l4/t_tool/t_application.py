...
from engine.l2_os.application import Window, OpeningTool, WindowMap, InteractionTool, Application
...
class SimpleOpeningTool(OpeningTool):
...
class SimpleWriteTool(InteractionTool):
    def __init__(self, window_map: WindowMap):
...
class TextApplication(Application):
    ...
    def create_open_tool(self) -> SimpleOpeningTool:
        return SimpleOpeningTool(window_map=self.window_map)
...
class ApplicationTest(Unittest):
    ...
    def test_open_window(self):
        ...
        self.assertEqual(len(self.app.window_map), initial_window_count + 1, "OpeningTool should add a new window")
