...
class ExampleEntries:
    def __init__(self):
        self.introduction = Message.user(text='Hi there, pleased to meet you! Who are you and what is your expertise?')
        self.repetition = Message.user(text='Can you please repeat what I said above in its entirety?')

        fpath = ExampleFiles.lend_png().fpath
        img_io = ImageFile(fpath=fpath)
        img_content = img_io.read()
        self.image_entry = Message.user(text='Can you describe what\'s in this image?', image=img_content)

        self.welcome_request = Message.user(
            text='## Automated message: Please greet our eight guests and welcome them to our home! You need only do this once, every guest will see it.')
        self.notify_chef = Message.user(text='Also please notify the chef that we need food for eight people.')
        self.write_num_guests = Message.user(
            text='Please tell me how many guests there are without invoking the display, then display a (single) warm welcome message')
