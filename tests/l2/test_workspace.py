...
class TestActionExecution(WorkspaceTest):
    def test_tool_execution(self):
        self.workspace.open_action(uri='test_path')
        add = self.workspace.tool_dict['add']
        reset = self.workspace.tool_dict['reset']

        add_json_str = '{"msg": "New text"}'
        tool_call = ToolCall(json_str=add_json_str)

        add.handle(tool_call=tool_call)
        self.assertIn('New text', self.workspace.window.workspace[0].get_text())
        self.log(f'Window context before reset: {self.workspace.window.get_entry()}')

        reset_json_str = '{}'
        tool_call = ToolCall(json_str=reset_json_str)
        reset.handle(tool_call)
        self.assertEqual('', self.workspace.window.workspace[0].get_text())
        self.log(f'Window context after reset : {self.workspace.window.get_entry()}')
