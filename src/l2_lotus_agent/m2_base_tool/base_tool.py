...
class BaseTool:
    ...
    def handle_call(self, args_dict: dict):
        ...
        try:
            ...
            func_timeout(timeout=BaseTool.timout_in_sec, func=self.do)
            self.finish_log(f'Tool {self.name} completed execution')

        except FunctionTimedOut:
            self.finish_log(
                f'The tool {self.name} timed out without completing after {BaseTool.timout_in_sec} seconds. Aborting ...')
